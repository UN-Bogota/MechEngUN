...
#comp_ini = 'o2:1, n2:3, ch4:0.01'
comp_fuel = 'c12h26:1'

f = 3668. / 60.  # engine speed [1/s] (3000 rpm)
V_H = .406e-3  # displaced volume [m**3]
epsilon = 12.5 # compression ratio [-]
d_piston = 0.086  # piston diameter [m]
...
injector_open = 230. / 180. * np.pi
injector_close = 245. / 180. * np.pi
injector_mass = 0.8e-5  # kg
...
t = states.t
t_min=sim_n_revolutions/(2*f)
# pressure and temperature
xticks = np.arange(0.0, sim_n_revolutions/f, sim_n_revolutions/(4*f))
...
# p-V diagram
fig, ax = plt.subplots()
ax.plot(states.V[t > t_min] * 1000, states.P[t > t_min] / 1.e5)
ax.set_xlabel('$V$ [l]')
ax.set_ylabel('$p$ [bar]')
plt.show()
...
# T-S diagram
fig, ax = plt.subplots()
ax.plot(states.m[t > t_min] * states.s[t > t_min], states.T[t > t_min])
ax.set_xlabel('$S$ [J/K]')
ax.set_ylabel('$T$ [K]')
plt.show()
...
ax.plot(t, states('h2').X, label='H2')
...
species=['c12h26', 'o2', 'n2', 'co2', 'h2o', 'co', 'h2', 'oh', 'h2o2', 'ho2', 'h', 'o',
    'ch3o','ch2o', 'hco', 'ch2', 'ch3', 'ch4', 'c2h2', 'c2h3', 'c12h25', 'c12h25o2', 'c12ket', 'c10h21',
